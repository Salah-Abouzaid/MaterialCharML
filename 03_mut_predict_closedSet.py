...
parser = argparse.ArgumentParser(description='Closed Set Classifier Prediction')
...
###############################Closed Set Network Prediction###############################
# Build the network
print('==> Building network')

network = closedSetClassifier.closedSetClassifier(num_classes, dataset_config['sig_channels'],
                                              dataset_config['sig_length'], dropout=dataset_config['dropout'])

# Load network weights
checkpoint = torch.load(f'networks/weights/{args.dataset}/{args.dataset}_{model_num}_{args.name}closedSetClassifierTotalLoss.pth')
...
# Make predictions
logits, tan_loss = network(test_samples_tensor)
...
print('calculated score: ', -adjusted_scores.min())
prediction = np.argmin(adjusted_scores, axis=1)
predicted_class = known_classes[int(prediction)]
...
loss_tangent = tan_loss_scaler.inverse_transform(tan_loss.detach().cpu().numpy())
print('loss tangent: ', loss_tangent)

# Gather thickness-permittivity values for each prediction
thickness_permittivity_values = [labels[predicted_value] for predicted_value in clusters[predicted_class][0]]
